...
//     const sortFromUrl = urlparams.get('sort');
//     const orderFromUrl = urlparams.get('order');
//     const addressFromUrl = urlparams.get('address');  // Changed 'location' to 'address'
...
//         sort: sortFromUrl || 'created_at',
//         order: orderFromUrl || 'desc',
//         address: addressFromUrl || '',  // Changed 'location' to 'address'
//       });
//     }
...
//       setSidebarData({ ...sidebarData, searchTerm: e.target.value });
//     }
//     if (e.target.id === 'address') {  // Changed 'location' to 'address'
...
//     urlparams.set('sort', sidebarData.sort);
//     urlparams.set('order', sidebarData.order);
//     urlparams.set('address', sidebarData.address);  // Changed 'location' to 'address'
...
//               id="address"  // Changed 'location' to 'address'
//               className="border rounded-lg p-3 w-full"
...
//               className="text-green-700 p-7 text-center w-full uppercase hover:underline"
//             >
//               Show More
//             </button>
//           )}
...
export default function Search() {
  ...
  useEffect(() => {
    ...
    fetchListings();
  }, [location.search, API_BASE]);

  const handleChange = (e) => {
    if (["all", "rent", "sale"].includes(e.target.id)) {
      setSidebarData({ ...sidebarData, type: e.target.id });
    } else if (e.target.id === "searchTerm" || e.target.id === "address") {
  ...
  const onShowMoreClick = async () => {
    ...
    const urlparams = new URLSearchParams(location.search);
    urlparams.set("startIndex", startIndex);
    const res = await fetch(`${API_BASE}/api/listing/get?${urlparams.toString()}`);
    const data = await res.json();
    setListings([...listings, ...data]);
